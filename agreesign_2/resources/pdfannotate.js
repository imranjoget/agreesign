...
var PDFAnnotate = function (container_id, url, options = {}) {
  ...
  this.active_tool = 0; // 0 - Free hand, 1 - Pen, 2 - Text, 3 - Arrow, 4 - Rectangle, 5 - Text Signature, 6 - Draw Signature
  this.fabricObjects = [];
  ...
  this.initFabric = function () {
    ...
    canvases.each(function (index, el) {
      ...
      $(fabricObj.upperCanvasEl).on("click tap touchstart", fabricObj.upperCanvasEl, function(event){
  ...
  this.fabricClickHandler = function (event, fabricObj) {
    ...
    if (inst.active_tool == 2) {
      ...
      if (event.type == 'touchstart' || event.type == 'touchmove' || event.type == 'touchend' || event.type == 'touchcancel') {
        ...
        x = (touch.pageX - fabricObj.upperCanvasEl.getBoundingClientRect().left) / options.scale;
        y = (touch.pageY - fabricObj.upperCanvasEl.getBoundingClientRect().top) / options.scale;
      } else if (event.type == 'click') {
        console.log('mouse');
        x = (event.clientX - fabricObj.upperCanvasEl.getBoundingClientRect().left) / options.scale;
        y = (event.clientY - fabricObj.upperCanvasEl.getBoundingClientRect().top) / options.scale;
      }
      toolObj = new fabric.LabeledRect({
        left: x,
        top: y,
        width: 150,
        height: 100,
        ...
        stroke: 'rgb(22, 119, 255)', // Set the stroke color (e.g., red)
        strokeDashArray: [5, 5],
        strokeWidth: 1
      });
    }
...
PDFAnnotate.prototype.enableTextSignature = function () {
  var inst = this;
  var fabricObj = inst.fabricObjects[inst.active_canvas];
  inst.active_tool = 5;
  if (inst.fabricObjects.length > 0) {
...
PDFAnnotate.prototype.enableDrawSignature = function () {
  var inst = this;
  var fabricObj = inst.fabricObjects[inst.active_canvas];
  inst.active_tool = 6;
  if (inst.fabricObjects.length > 0) {
